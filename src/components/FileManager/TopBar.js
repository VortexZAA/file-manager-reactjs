  ...
  return (
    <div className='FileManager-TopBar'>
      <div className='TopBar-Left'>
        <button className='TopBar-Button Icon-Button' type='button' disabled={!currentPath} onClick={() => setCurrentPath('')} title={labels['home']}>
          <FaHome />
        </button>
        <button
        ...
        <button className='TopBar-Button Icon-Button' type='button' title={labels['reload']} onClick={() => reload()}>
          {<FaSyncAlt />}
        </button>
        <input
          key={currentPath}
      ...
      <div hidden className=' TopBar-Right'>
        <input ref={uploadInputRef} type='file' onChange={onFileSelect} hidden />
        {enabledFeatures.indexOf('createDirectory') !== -1 && (
          <button className='TopBar-Button' type='button' onClick={() => onCreateDirectory()}>
            {<FaFolderPlus />} {labels['createDirectory']}
          </button>
        )}
        {enabledFeatures.indexOf('uploadFiles') !== -1 && (
          <button className='TopBar-Button' type='button' onClick={() => uploadInputRef.current && uploadInputRef.current.click()}>
